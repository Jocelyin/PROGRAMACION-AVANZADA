...
public class GenerarVentaVista extends JPanel { 
    ...
    public GenerarVentaVista() {
        setLayout(new BorderLayout(5, 5)); // Ahora usamos setLayout en el JPanel
    ...
    // Métodos para interactuar con la vista (obtener texto de búsqueda, agregar/eliminar filas de la tabla, actualizar total, etc.)
    ...
    public void mostrarDialogoCantidad(Producto producto, ActionListener listener) {
        ...
        if (dialogoCantidad == null) {
        	dialogoCantidad = new JDialog((Frame) SwingUtilities.getWindowAncestor(this), "Ingresar Cantidad", true);
            dialogoCantidad.setLayout(new FlowLayout());
            txtCantidad = new JTextField(5);
            btnAgregarCantidad = new JButton("Agregar");
            btnAgregarCantidad.setActionCommand("agregar_cantidad"); // Nuevo comando
            ...
            dialogoCantidad.setLocationRelativeTo(this);
            // Agregar el listener al botón del diálogo aquí
            btnAgregarCantidad.addActionListener(listener);
        } else {
            txtCantidad.setText("1"); // Resetear la cantidad por defecto
        }

...
public class ReducirStockController implements ActionListener {
    ...
    private void reducirStockProducto() {
        ...
        if (cantidadReducir > productoExistente.getStock()) {
            JOptionPane.showMessageDialog(vista, "No hay suficiente stock para reducir " + cantidadReducir + " unidades del producto " + 
        codigoProducto + ". Stock actual: " + productoExistente.getStock(), "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        int nuevoStock = productoExistente.getStock() - cantidadReducir;
        if (productoDAO.actualizarStock(codigoProducto, nuevoStock)) {
            JOptionPane.showMessageDialog(vista, "Stock del producto " + codigoProducto + " reducido a " + nuevoStock + ".", "Éxito", 
